...
![image-20260104093626539](assets/image-20260104093626539.png)

![image-20260104093905306](assets/image-20260104093905306.png)

- 隔离后

![image-20260104093930634](assets/image-20260104093930634.png)

![image-20260104093954969](assets/image-20260104093954969.png)
...
```java
package com.autumnnook.controller;
...
public class MenuController {
    ...
    @HystrixCommand(
        commandProperties = {
            // 隔离策略，使用信号量隔离，默认值是 THREAD（线程池隔离）
        ...
        fallbackMethod = "getMenuByIdFallback"
    )
    public Menu getMenuById() {
        logger.info("[getMenuById] 当前线程：{}", Thread.currentThread().getName());
        return new Menu("1", "Espresso", "3.0");
    ...
    // 🎯 降级方法
    public Menu getMenuByIdFallback() {
        logger.info("[getMenuById] 当前线程：{}", Thread.currentThread().getName());
        return new Menu("1", "托底数据", "3.0");
    }
}
```

然后我们同样的通过 JMeter 压测工具发起请求，通过日志我们可以看到，当信号量不足时，则会触发降级方法。

![image-20251229231357442](assets/image-20251229231357442.png)

### 线程池隔离 VS 信号量隔离

下面我们对比以下线程池隔离策略和信号量隔离策略的相同点和不同点。

|  隔离方式  | 是否支持超时 | 是否支持熔断 |        隔离原理        |     是否异步调用     | 资源消耗 |
| :--------: | :----------: | :----------: | :--------------------: | :------------------: | :------: |
| 线程池隔离 |      ✔️       |      ✔️       | 每个服务单独使用线程池 |    支持同步/异步     |    大    |
| 信号量隔离 |      ❌       |      ✔️       |   通过信号量的计数器   | 同步调用，不支持异步 |    小    |

**🎯线程池隔离**

- 请求线程和调用服务线程不是同一条线程；
- 支持超时，可以直接返回；
- 支持熔断，当线程池达到最大线程数之后，再请求会触发 `fallback` 接口进行熔断；
- 隔离原理：每个服务单独使用线程池；
- 支持同步/异步两种方式；
- <span style="color:#ff7043;">资源消耗过大，大量线程的上下文切换、排队、调度等，容易造成机器负载高；</span>
- <span style="color:#ff7043;">无法传递 HTTP 请求头；</span>

**🧇信号量隔离**

- 请求线程和调用服务线程是同一个线程；
- <span style="color:#ff7043;">不支持超时；</span>
- 支持熔断，当信号量达到 `maxConcurrentRequests` 后，再请求会触发 `fallback` 方法进行熔断降级；
- 隔离原理：通过信号量的计数器；
- 同步调用（阻塞线程），<span style="color:#ff7043;">不支持异步</span>；
- 资源消耗小，只是一个计数器；
- 可以传递 HTTP 请求头；

### 总结

对于两种隔离策略，我们需要根据不同的场景选择使用相应的的策略，大致可归结为以下两点：

- 请求并发大，且耗时长（如计算量大、或是操作关系型数据库），一般采用线程池隔离。这样可以保证大量的线程可用，不会由于服务原因一直处于阻塞/等待状态，快速失败返回。还有就是对依赖服务的网络请求的调用及访问，会涉及 `timeout` 超时这种问题的都使用线程池隔离。
- 请求并发大，耗时短（计算小、或操作缓存），采用信号量隔离，因为这类服务的返回通常会非常的快，不会占用线程太长时间，而且也减少了线程切换的开销，提高了缓存服务的效率。还有就是适合访问不是对外部依赖的服务，而是对内部的一些比较复杂的业务逻辑的访问，像这种访问系统内部的代码，不会涉及任何的网络请求，做信号量的普通限流就可以了，因为不需要去捕获 `timeout` 类似的问题，并发量突然太高，稍微耗时一些就会导致大量线程阻塞，所以进行一个基本的资源隔离和访问，避免内部复杂的低效率的代码，导致大量的线程被宕住。
